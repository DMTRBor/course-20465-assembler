    ...
    while (curr_line != NULL) {
        ...
        switch (line_arg_type) {
            ...
            case LABEL:
                ...
                /* detect label type */
                if (is_extern) {
                    label->name = duplicate_str(curr_line->line);  /* label coming from extern directive */
                    set_label_fields(label, EXTERNAL, EXTERN_LABEL_ADDRESS);
                }
                else { /* set new label name */
                    if ((label->name = get_label_name(&curr_line->line)) == NULL) {
                        fprintf(stderr, "Error in line %d - label parsing error for: %s \n",
                                line_number, curr_line->line);
                        error_flag = TRUE;
                        break;  /* failed line go to next line */
                    }
				}
            ...
            case DIRECTIVE:
                ...
                if (is_expected_directive(curr_line->line, ENTRY_DIRECTIVE)) {
                    ...
                    /* check if has subsequent commas */
                    if (has_mat_direc_subseq_commas(curr_line->line, line_number)) {
                        error_flag = TRUE;
                        break;
                    }
