...
int encode_matrix_row_col(Word *word, char *op_line, int line_number) {
    char *op_args, *op_copy;
    int arg_id = 0;  /* arg index */

    /* copy line for processing */
    op_copy = duplicate_str(op_line);
    ...
    while (mem_unit != NULL) {
        if (mem_unit->label_op_name != NULL) {  /* label found */
            ...
            while (label != NULL) {
                ...
                label = label->next;  /* go to next label */
            }
        ...
        mem_unit = mem_unit->next;  /* go to next memory unit */
    }
