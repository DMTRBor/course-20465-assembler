...
char* duplicate_str(const char *string) {
    ...
    /* allocate memory for string duplication */
    /* +1 for null terminator */
    if ((dup = malloc(strlen(string) + 1)) == NULL) {
        fprintf(stderr, "Error allocating memory for string duplication\n");
        return NULL;
    }

    strcpy(dup, string);  /* copy string content */
    return dup;
}
...
int is_label_reserved_name(char *arg) {
    char label_name[MAX_LINE_LEN];
    int id, arg_len = strlen(arg);  /* get string length */

    /* copy characters to buffer except the last one, that is ':' */
    for (id = 0; id < arg_len - 1 && id < MAX_LINE_LEN - 1; id++) {
        label_name[id] = arg[id];
    }
    label_name[id] = NULL_TERMINATOR; /* terminate the string */

    /* check if it's a reserved name */
    return (is_operation(label_name) || is_directive(label_name) || is_register(label_name));
}
...
char* get_label_name(char **line_with_label) {
    ...
    if (lbl_end == NULL)  /* error in label name parsing */
        return NULL;

    /* get label length */
    len = lbl_end - *line_with_label;
    if ((label_name = (char *)malloc(len)) == NULL) {
        fprintf(stderr, "Error in memory allocation for label name\n");
...
int str_to_int(char *num_str, int *num_int, int line_number) {
    char *end;
    long value;
...
int get_mat_size(char *mat_size_str) {
    ...
    /* copy line for processing */
    size_copy = duplicate_str(mat_size_str);
    /* tokenize with different delimiters */
