...
void word_to_base4(Word encoded_value, char *converted) {
    ...
    /* combine Word fields into a single 10-bit integer */
    word_decimal = WORD_TO_DECIMAL(encoded_value);
...
int build_object_file(char *filename, unsigned int ICF, unsigned int DCF, MemoryUnit **mem_map) {
    ...
    char obj_content[MAX_FILE_BUFFER] = "";  /* buffer to store full machine code */
    char line[MAX_LINE_LEN];  /* buffer for machine code line  */
